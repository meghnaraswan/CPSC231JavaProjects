...
public abstract class Affiliate implements Printable, Comparable<Affiliate> {
  ...
  */
  public void print(){
    System.out.println(this.toString()); //calls toString method for formatting
  }

  /**
  * toString method to format the affiliate information correctly
  */
  public String toString(){
    String ret = "";
    ret += String.format("%s,%d,%s,%s,%d", m_address, m_age, m_name, m_phone_num, m_year);
    return ret;
  }

  /**
  * Comparable interface compares the year affiliate came to Champman
  * The longer they have been at Chapman, the more seniority they possess
  */
  public int compareTo(Affiliate a){
    int ret = 0;
    if (this.m_year < a.m_year){
      ret = -1; //object making the call is "greater than"
    }
    else if (this.m_year > a.m_year){
      ret = 1; //object making the call is "less than"
    }
    else{
      ret = 0;
    }
